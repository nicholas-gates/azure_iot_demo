...
#define TELEMETRY_PROP_NAME_RANDOMNUMBER                "randomNumber"
...
static int generate_telemetry_payload(uint8_t* payload_buffer, size_t payload_buffer_size, size_t* payload_buffer_length)
{
  ...
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_RANDOMNUMBER));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding random number property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, randomNumber);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding random number property value to telemetry payload. ");
